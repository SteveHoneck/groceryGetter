...
//Reducer function to handle the 'item' part of the Redux state. A part of the Redux state is created and called 'item' and exported.
export const item = (state = {itemArray: [], errMess: null}, action) => {//Reducers take 2 parameters: 1st is the state that is already in the store (The first time the reducer is called, the state will not exist. Default function prameter syntax (  state = {itemArray: [], errMess: null})  ) will be used to initialize the part of the state handled by this reducer if the state does not exist, this initializes the 'item' part of the state as an object containing an empty array called 'itemArray' and the 'errMess' (which is intended to hold an error message from the server if needed) as null ). 2nd parameter is an action object. The body of the reducer funciton will check for the type of the action and return the state.
    switch (action.type){
        case ActionTypes.LOAD_ITEMS: //Action of loading the 'item' portion of the Redux state (which contains the 'itemArray' and an 'errMess')from the server.
            return {...state, itemArray: action.payload, errMess: null}; // Return a new state updated with the 'itemArray' that was loaded from the server and stored in 'action.payload' and the 'errMess' set to 'null' because since something was sucessfully loaded from the server, there must be no error message.
